...
interface SuggestionInputProps {
  fetchSuggestions?: (query: string) => Promise<string[] | CollegeSuggestion[]>;
  suggestions?: string[] | CollegeSuggestion[];
  placeholder?: string;
  value?: string;
  onChange?: (value: string) => void;
  onSelect?: (suggestion: string | CollegeSuggestion) => void;
  className?: string;
...
export function SuggestionInput({
  ...
  const handleSuggestionClick = (suggestion: string | CollegeSuggestion) => {
    const displayText =
      typeof suggestion === "string" ? suggestion : suggestion.name;
  ...
  return (
    <div className="relative w-full">
      ...
      {isLoading && (
        <div className="absolute left-0 right-0 top-full z-50 mt-1 rounded-md border bg-popover p-3 text-center text-sm text-muted-foreground shadow-md">
          Loading suggestions...
        </div>
      )}

      {error && (
        <div className="absolute left-0 right-0 top-full z-50 mt-1 rounded-md border bg-destructive/10 p-3 text-center text-sm text-destructive shadow-md">
          {error}
        </div>
      )}
        ...
        >
          {filteredSuggestions.map((suggestion, index) => {
            ...
            const displayName =
              typeof suggestion === "string" ? suggestion : suggestion.name;
            const location =
              typeof suggestion === "string" ? null : suggestion.location;
            ...
            return (
              <div
                ...
                className={cn(
                  "cursor-pointer bg-white px-3 py-2 hover:bg-accent hover:text-accent-foreground",
